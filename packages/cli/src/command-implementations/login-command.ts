import * as client from '@mlagent/client'
  ...
  public async run(): Promise<void> {
    ...
    const promptedWorkspaceId = await this.globalCache.sync('workspaceId', this.argv.workspaceId, async (defaultId) => {
      ...
      const userWorkspaces = await paging
        ...
        .catch((thrown) => {
          throw errors.MlagentCLIError.wrap(thrown, 'Could not list workspaces')
        })
    ...
    await api.testLogin().catch((thrown) => {
      throw errors.MlagentCLIError.wrap(thrown, 'Login failed. Please check your credentials')
    })
