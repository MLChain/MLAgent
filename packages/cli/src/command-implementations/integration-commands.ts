...
export class GetIntegrationCommand extends GlobalCommand<GetIntegrationCommandDefinition> {
  public async run(): Promise<void> {
    ...
    if (parsedRef.type === 'path') {
      throw new errors.MlagentCLIError('Cannot get local integration')
    }
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, `Could not get integration ${this.argv.integrationRef}`)
    }

    throw new errors.MlagentCLIError(`Integration ${this.argv.integrationRef} not found`)
  }
}
...
export class ListIntegrationsCommand extends GlobalCommand<ListIntegrationsCommandDefinition> {
  public async run(): Promise<void> {
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, 'Could not list integrations')
    }
  }
}
...
export class DeleteIntegrationCommand extends GlobalCommand<DeleteIntegrationCommandDefinition> {
  public async run(): Promise<void> {
    ...
    if (parsedRef.type === 'path') {
      throw new errors.MlagentCLIError('Cannot delete local integration')
    }
    ...
    if (!integration) {
      ...
      if (publicIntegration) {
        throw new errors.MlagentCLIError(`Integration ${this.argv.integrationRef} does not belong to your workspace`)
      }
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, `Could not delete integration ${this.argv.integrationRef}`)
    }
