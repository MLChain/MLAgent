...
export class GetInterfaceCommand extends GlobalCommand<GetInterfaceCommandDefinition> {
  public async run(): Promise<void> {
    ...
    if (parsedRef.type === 'path') {
      throw new errors.MlagentCLIError('Cannot get local interface')
    }
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, `Could not get interface ${this.argv.interfaceRef}`)
    }

    throw new errors.MlagentCLIError(`Interface ${this.argv.interfaceRef} not found`)
  }
}
...
export class ListInterfacesCommand extends GlobalCommand<ListInterfacesCommandDefinition> {
  public async run(): Promise<void> {
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, 'Could not list interfaces')
    }
  }
}
...
export class DeleteInterfaceCommand extends GlobalCommand<DeleteInterfaceCommandDefinition> {
  public async run(): Promise<void> {
    ...
    if (parsedRef.type === 'path') {
      throw new errors.MlagentCLIError('Cannot delete local interface')
    }
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, `Could not delete interface ${this.argv.interfaceRef}`)
    }
