...
export type ConfigurableGlobalPaths = { mlagentHomeDir: string; cliRootDir: utils.path.AbsolutePath }
...
class GlobalPaths extends utils.path.PathStore<keyof AllGlobalPaths> {
  public constructor(argv: CommandArgv<GlobalCommandDefinition>) {
    const absMlagentHome = utils.path.absoluteFrom(utils.path.cwd(), argv.mlagentHome)
    super({
      cliRootDir: consts.cliRootDir,
      mlagentHomeDir: absMlagentHome,
      ..._.mapValues(consts.fromHomeDir, (p) => utils.path.absoluteFrom(absMlagentHome, p)),
...
export abstract class GlobalCommand<C extends GlobalCommandDefinition> extends BaseCommand<C> {
  ...
  protected override async bootstrap() {
    ...
    this.logger.log(`Mlagent CLI ${versionText}`, { prefix: '🤖' })

    await this._notifyUpdateCli()

    const paths = this.globalPaths
    if (paths.abs.mlagentHomeDir !== consts.defaultMlagentHome) {
      this.logger.log(`Using custom mlagent home: ${paths.abs.mlagentHomeDir}`, { prefix: '🏠' })
    }
  }
  ...
  protected async ensureLoginAndCreateClient(credentials: YargsConfig<typeof config.schemas.credentials>) {
    ...
    if (apiUrl !== consts.defaultMlagentApiUrl) {
  ...
  private _notifyUpdateCli = async (): Promise<void> => {
    try {
      ...
      const pkgJson = await this.readPkgJson()
      if (!pkgJson.version) {
        throw new errors.MlagentCLIError('Could not find version in package.json')
      }
      ...
      const err = errors.MlagentCLIError.map(thrown)
  ...
  protected async readPkgJson(): Promise<utils.pkgJson.PackageJson> {
    ...
    if (!pkgJson) {
      throw new errors.MlagentCLIError(`Could not find package.json at "${cliRootDir}"`)
    }
