...
export class DevCommand extends ProjectCommand<DevCommandDefinition> {
  ...
  public async run(): Promise<void> {
    ...
    if (projectDef.type === 'interface') {
      throw new errors.MlagentCLIError('This feature is not available for interfaces.')
    }
    ...
    if (urlParseResult.status === 'error') {
      throw new errors.MlagentCLIError(`Invalid tunnel URL: ${urlParseResult.error}`)
    }
    ...
    supervisor.events.on('connected', ({ tunnel }) => {
      ...
      tunnel.events.on('request', (req) => {
        ...
        void this._forwardTunnelRequest(`http://localhost:${port}`, req)
          ...
          .catch((thrown) => {
            const err = errors.MlagentCLIError.wrap(thrown, 'An error occurred while handling request')
    ...
    try {
      ...
      throw errors.MlagentCLIError.wrap(thrown, 'An error occurred while running the dev server')
    } finally {
      if (worker.running) {
        await worker.kill()
      }
  ...
  private _restart = async (api: ApiClient, worker: Worker, tunnelUrl: string) => {
    try {
      ...
      const error = errors.MlagentCLIError.wrap(thrown, 'Build failed')
  ...
  private _checkSecrets(integrationDef: sdk.IntegrationDefinition) {
    ...
    if (newSecrets.length > 0) {
      throw new errors.MlagentCLIError('Secrets were added while the server was running. A restart is required.')
    }
  }
  ...
  private _spawnWorker = async (env: Record<string, string>, port: number) => {
    ...
    const worker = await Worker.spawn(
      ...
      throw errors.MlagentCLIError.wrap(thrown, 'Could not start dev worker')
    })
  ...
  private async _deployDevIntegration(
    ...
    if (devId) {
      const resp = await api.client.getIntegration({ id: devId }).catch(async (thrown) => {
        const err = errors.MlagentCLIError.wrap(thrown, `Could not find existing dev integration with id "${devId}"`)
    ...
    if (integration) {
      ...
      const resp = await api.client.updateIntegration(updateIntegrationBody).catch((thrown) => {
        throw errors.MlagentCLIError.wrap(thrown, `Could not update dev integration "${integrationDef.name}"`)
      })
      integration = resp.integration
    } else {
      const resp = await api.client.createIntegration({ ...createIntegrationBody, dev: true }).catch((thrown) => {
        throw errors.MlagentCLIError.wrap(thrown, `Could not deploy dev integration "${integrationDef.name}"`)
      })
      integration = resp.integration
    }
  ...
  private async _deployDevBot(api: ApiClient, externalUrl: string): Promise<void> {
    ...
    if (devId) {
      const resp = await api.client.getBot({ id: devId }).catch(async (thrown) => {
        const err = errors.MlagentCLIError.wrap(thrown, `Could not find existing dev bot with id "${devId}"`)
        this.logger.warn(err.message)
    ...
    if (!bot) {
      ...
      const resp = await api.client
        ...
        .catch((thrown) => {
          throw errors.MlagentCLIError.wrap(thrown, 'Could not deploy dev bot')
        })
