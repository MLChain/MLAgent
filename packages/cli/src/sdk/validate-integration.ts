...
export const validateIntegrationDefinition = (i: sdk.IntegrationDefinition): void => {
  ...
  if (invalidEventNames.length) {
    throw new errors.MlagentCLIError(`The following event names are not in camelCase: ${invalidEventNames.join(', ')}`)
  }

  const invalidStateNames = _nonCamelCaseKeys(states ?? {})
  if (invalidStateNames.length) {
    throw new errors.MlagentCLIError(`The following state names are not in camelCase: ${invalidStateNames.join(', ')}`)
  }
}
