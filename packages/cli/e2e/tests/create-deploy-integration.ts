...
export const createDeployIntegration: Test = {
  ...
  handler: async ({ tmpDir, dependencies, workspaceHandle, logger, ...creds }) => {
    const mlagentHomeDir = pathlib.join(tmpDir, '.mlagenthome')
    ...
    await utils.fixMlagentDependencies({ workDir: integrationDir, target: dependencies })
