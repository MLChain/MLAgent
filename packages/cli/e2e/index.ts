...
const configSchema = {
  ...
  apiUrl: {
    type: 'string',
    default: consts.defaultMlagentApiUrl,
  },
  tunnelUrl: {
    type: 'string',
  ...
  sdkPath: {
    type: 'string',
    description: 'Path to the Mlagent SDK to install; Allows using a version not released on NPM yet.',
  },
  clientPath: {
    type: 'string',
    description: 'Path to the Mlagent Client to install; Allows using a version not released on NPM yet.',
  },
...
const main = async (argv: YargsConfig<typeof configSchema>): Promise<never> => {
  ...
  const dependencies = { '@mlagent/sdk': argv.sdkPath, '@mlagent/client': argv.clientPath } satisfies Record<
    string,
    string | undefined
  >
