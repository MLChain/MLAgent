import { axios } from '@mlagent/client'
import { RuntimeError } from '@mlagent/sdk'
import { AssertionError, ok } from 'assert'
import _ from 'lodash'
import { Context, Markup, Telegraf, Telegram } from 'telegraf'
import { PhotoSize, Update, User } from 'telegraf/typings/core/types/typegram'
import { Card, AckFunction, Logger, MessageHandlerProps, MlagentMessage, TelegramMessage } from './types'
...
export const convertTelegramMessageToMlagentMessage = async ({
  message,
  telegram,
}: {
  message: TelegramMessage
  telegram: Telegram
}): Promise<MlagentMessage> => {
  if ('photo' in message) {
